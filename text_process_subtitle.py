...
def preprocess_data(input_file):
    ...
    df = df[(df['Language'] == '英语(自动生成)' ) | ( df['Language'] =='英语')]  # 只保留英语
    df['text_length'] = df['Text'].apply(len)
    df_filtered = df[df['text_length'] >= 10]  # 过滤掉长度小于10的评论
    df_filtered['Upload_time'] = pd.to_datetime(df_filtered['Upload_time'], format='%Y%m%d')
    df_filtered['year'] = df_filtered['Upload_time'].dt.year
...
if __name__ == '__main__':
    ...
    input_file = 'segmented_subtitles_with_upload_time.csv'  # 输入文件路径
    df_new = preprocess_data(input_file)  # 预处理数据
    df_new.to_csv('cleaned_subtitle.csv', index=False, encoding='utf-8-sig')  # 保存预处理后的数据
    print("预处理完成，已保存到 cleaned_subtitle.csv")
